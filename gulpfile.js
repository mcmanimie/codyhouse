...
gulp.task('dist', async function(){
  ...
  await moveFonts();
  // copy all font files inside main folder to the dist folder
  await moveVideo();
  // copy all video files inside main folder to the dist folder
  await moveContent();
  // copy all video files inside main folder to the dist folder
  await moveRedirect();
  await movePDF();
...
function moveVideo() {
  return new Promise(function(resolve, reject) {
    var stream = gulp.src(['main/assets/video/**'], { allowEmpty: true })
    .pipe(gulp.dest(assetsFolder+'video'));
...
function moveRedirect() {
  return new Promise(function(resolve, reject) {
    var stream = gulp.src('main/_redirects')
    .pipe(gulp.dest(distFolder));
